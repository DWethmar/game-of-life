...
aliveCellPercentage = getIntParam(
  "start-percentage",
  aliveCellPercentage,
  gridSize * gridSize
);
...
const startWithCells = Math.floor(
  (aliveCellPercentage / 100) * (gridSize * gridSize)
);
...
const updateFPSCounter = _.debounce(
  (fps: number) => {
    if (FPSCounter) {
...
const UpdateAliveCellsCounter = _.debounce(
  (alive: number, total: number) => {
    if (AliveCellsCounter) {
      AliveCellsCounter.innerHTML = `${alive.toString()}/${total.toString()} - ${(
        (alive / total) *
        100
      )
        .toFixed(2)
...
function step() {
  scene.children.forEach((c) => c.destroy());
  grid.step();
